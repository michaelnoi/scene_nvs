"""
wild mixture of
https://github.com/lucidrains/denoising-diffusion-pytorch/blob/7706bdfc6f527f58d33f84b7b522e61e6e3164b3/denoising_diffusion_pytorch/denoising_diffusion_pytorch.py # noqa
https://github.com/openai/improved-diffusion/blob/e94489283bb876ac1477d5dd7709bbbd2d9902ce/improved_diffusion/gaussian_diffusion.py # noqa
...
class DDPM(pl.LightningModule):
    ...
    def __init__(
        ...
        print(
            f"{self.__class__.__name__}: Running in \
                {self.parameterization}-prediction mode"
        )
    ...
    def register_schedule(
        ...
        # below: log calculation clipped because the posterior variance is
        # 0 at the beginning of the diffusion chain
        self.register_buffer(
    ...
    def forward(self, x, *args, **kwargs):
        ...
        # assert h == img_size and \
        # w == img_size, f'height and width of image must be {img_size}'
        t = torch.randint(
...
class LatentDiffusion(DDPM):
    ...
    def get_first_stage_encoding(self, encoder_posterior):
        if isinstance(encoder_posterior, DiagonalGaussianDistribution):
            ...
            raise NotImplementedError(
                f"encoder_posterior of type '{type(encoder_posterior)}' \
                    not yet implemented"
            )
    ...
    def _rescale_annotations(self, bboxes, crop_coordinates):  # TODO: move to dataset
        def rescale_bbox(bbox):  # TODO: find what clamp was supposed to do here
            x0 = (bbox[0] - crop_coordinates[0]) / crop_coordinates[
                2
            ]  # clamp((bbox[0] - crop_coordinates[0]) / crop_coordinates[2])
            y0 = (bbox[1] - crop_coordinates[1]) / crop_coordinates[
                3
            ]  # clamp((bbox[1] - crop_coordinates[1]) / crop_coordinates[3])
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if self.model.conditioning_key and self.cond_stage_key in [
                ...
                # as we are operating on latents,
                # we need the factor from the original image size to the
                # spatial latent size to properly rescale
                # the crops for regenerating the bbox annotations
                num_downs = self.first_stage_model.encoder.num_resolutions - 1
                rescale_latent = 2 ** (num_downs)

                # get top left postions of patches as conforming
                # for the bbbox tokenizer, therefore we need to rescale the tl
                # patch coordinates to be in between (0,1)
                ...
                # patch_values = [(np.arange(x_tl,min(x_tl+ks, 1.)), \
                #   np.arange(y_tl,min(y_tl+ks, 1.))) \
                # for x_tl, y_tl in tl_patch_coordinates]

                # tokenize crop coordinates for the bounding boxes
...
class LatentUpscaleDiffusion(LatentDiffusion):
    def __init__(self, *args, low_scale_config, low_scale_key="LR", **kwargs):
        ...
        # assumes that neither the cond_stage
        # nor the low_scale_model contain trainable params
    ...
    def log_images(
        ...
        **kwargs,
        ...
        log["reconstruction"] = xrec
        log["x_lr"] = x_low
        noise_levels = "-".join(map(lambda x: str(x), list(noise_level.cpu().numpy())))
        log[f"x_lr_rec_@noise_levels{noise_levels}"] = x_low_rec
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode"):
                xc = self.cond_stage_model.decode(c)
                log["conditioning"] = xc
        ...
        if unconditional_guidance_scale > 1.0:
            ...
            # maybe guide away from empty text label and
            # highest noise level and maximally degraded zx?
            uc = dict()
            for k in c:
                if k == "c_crossattn":
...
class LatentInpaintDiffusion(LatentDiffusion):
    """
    can either run as pure inpainting model (only concat mode)
    or with mixed conditionings, e.g. mask as concat and text via cross-attn.
    ...
    def __init__(
        ...
        # if model was trained without concat mode before
        # and we would like to keep these channels:
            ...
            if exists(self.finetune_keys) and k in self.finetune_keys:
                ...
                for name, param in self.named_parameters():
                    if name in self.finetune_keys:
                        print(
                            f"modifying key '{name}' and keeping its original \
                                {self.keep_dims} (channels) dimensions only"
                        )
        ...
        **kwargs,
        ...
        if sample:
            ...
            with ema_scope("Sampling"):
                ...
                # self.sample(cond=c, batch_size=N, return_intermediates=True)
            x_samples = self.decode_first_stage(samples)
            log["samples"] = x_samples
