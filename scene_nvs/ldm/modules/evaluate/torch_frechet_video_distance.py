# based on https://github.com/universome/fvd-comparison/blob/master/compare_models.py
import glob
...
def open_url(
    ...
    """Download the given URL and \
        return a binary-mode file object to access the data."""
...
def get_data_from_str(input_str, nprc=None):
    ...
    if nprc is None:
        try:
            ...
            print(
                "WARNING: cpu_count() not avlailable, \
                    using only 1 cpu for video loading"
            )
            nprc = 1
...
def compute_statistics(
    ...
    if only_ref:
        if not isvideo(videos_real):
            # if not is video we assume to have numpy arrays of shape
            # (n_vids, t, h, w, c) in range [0,255]
        ...
        for ref_v, sample_v in tqdm(
            ...
            # print(ref_v.shape)
            # ref_v = torch.nn.functional.interpolate(
            #   ref_v,
            #   size=(sample_v.shape[2], 256, 256),
            #   mode='trilinear',
            #   align_corners=False
            # )
            # sample_v = torch.nn.functional.interpolate(
            #   sample_v,
            #   size=(sample_v.shape[2], 256, 256),
            #   mode='trilinear',
            #   align_corners=False
