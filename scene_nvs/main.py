...
def nondefault_trainer_args(opt):
    ...
    # Add arguments to parser instead of Trainer.add_argparse_args(parser)
    # which is outdated since lightning 2.0.0
    parser.add_argument('--benchmark', type=bool, default=True)
    parser.add_argument('--val_check_interval', type=int, default=5000000)
...
# may be not needed, check if custom dataset works and TODO: make it from config
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def _train_dataloader(self):
        is_iterable_dataset = False # isinstance(self.datasets['train'], Txt2ImgIterableBaseDataset)
...
class SingleImageLogger(Callback):
    ...
    def _wandb(self, pl_module, images, batch_idx, split):
        for k in images:
            ...
            tag = f"{split}/{k}"
            wandb.log({tag: [wandb.Image(grid.permute(1, 2, 0).numpy() * 255)]}, step=pl_module.global_step)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not "gpus" in trainer_config and not "devices" in trainer_config:
            ...
            rank_zero_print("Running on CPU")
            cpu = True
        else:
            devices = trainer_config["devices"] if "devices" in trainer_config else trainer_config["gpus"]
            rank_zero_print(f"Running on {devices} GPUs")  # {gpuinfo}")
            cpu = False
        ...
        torch.set_float32_matmul_precision('medium')  # TODO: change, check if needed
        ...
        default_logger_cfgs = {
            ...
            # "testtube": {
            #     "target": "lightning.loggers.TestTubeLogger",
            #     "params": {
            #         "name": "testtube",
            #         "save_dir": logdir,
            #     }
        ...
        default_callbacks_cfg = {
            ...
            "image_logger": {
                ...
                "params": {
                    "batch_frequency": 250, # TODO: make 750 again
                    "max_images": 4,
                    "clamp": True
                }
            },
        ...
        if MULTINODE_HACKS:
            ...
            # from lightning.plugins.environments import SLURMEnvironment
